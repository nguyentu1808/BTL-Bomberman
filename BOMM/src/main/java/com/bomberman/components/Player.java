...
public class Player extends Component {
    ...
    public Player() {
        ...
        onCollisionBegin(Type_Game.PLAYER, Type_Game.SPEED_ITEM, (p, speed_i) -> {
        ...
        onCollisionBegin(Type_Game.PLAYER, Type_Game.BOMB_ITEM, (p, bombs_t) -> {
            bombs_t.removeFromWorld();
            inc("score", SCORE_ITEM);
            inc("bomb", 1);
        });

        onCollisionBegin(Type_Game.PLAYER, Type_Game.FLAME_ITEM, (p, flame_i) -> {
            flame_i.removeFromWorld();
            inc("score", SCORE_ITEM);
            inc("flame", 1);
        });

        onCollisionBegin(Type_Game.PLAYER, Type_Game.FLAME_PASS_ITEM, (p, flame_pass_i) -> {
